...
class TriStageLRScheduler(LearningRateScheduler):
    ...
    def _decide_stage(self):
        if self._step_count < self.warmup_steps:
            return 0, self._step_count

        offset = self.warmup_steps

        if self._step_count < offset + self.hold_steps:
            return 1, self._step_count - offset

        offset += self.hold_steps

        if self._step_count <= offset + self.decay_steps:
            # decay stage
            return 2, self._step_count - offset

        offset += self.decay_steps

        return 3, self._step_count - offset
