...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams, tokenizer):
        ...
        self.sampling_rate = hparams.sampling_rate
        self.spec_dir_name = hparams.spec_dir_name
    ...
    def get_audio(self, filename):
        ...
        # audio_max_value = torch.abs(audio).max()
        # audio_norm = audio / audio_max_value
        ...
        spec_dir = os.path.join(parent_dir, self.spec_dir_name)
        os.makedirs(spec_dir, exist_ok=True)

        spec_filename = os.path.join(spec_dir, f"{file_name_no_ext}.spec.pt")
        
        if os.path.exists(spec_filename):
            spec = torch.load(spec_filename)
        else:
            print(f"Spec file not found: {spec_filename}, generating new one...")
