...
CHARSIU_LANG_TO_CODE = {
    'en': 'eng-us', 'zh': 'zho-s', 'de': 'ger',
    'km': 'khm', 'af': 'afr',
}
...
def main():
    ...
    parser.add_argument("--output_dir", default="data/xphonebert")
    ...
    supported_langs = [lang for lang in langs if CHARSIU_LANG_TO_CODE.get(lang, '') != '']
    ...
    for lang in supported_langs:
        ...
        model = Text2PhonemeSequence(
            ...
            language=CHARSIU_LANG_TO_CODE[lang],
            is_cuda=cuda
        )
        ...
        for df, mode in zip([train, test], ['train', 'test']):
            print(f'Phonemizing {mode} data...')
            df['phoneme_seq'] = [model.infer_sentence(str(t)) for t in tqdm(df['text'])]
